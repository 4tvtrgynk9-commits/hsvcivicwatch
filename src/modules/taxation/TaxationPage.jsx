...
import data from "././taxation.data";
...
export default function TaxationPage() {
  const { liveIssues, loading } = useSupabaseModule("taxation");
  const [tabId, setTabId] = useState(data.tabs?.[0]?.id || "overview");
  const activeTab = data.tabs?.find((t) => t.id === tabId) || data.tabs?.[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("taxation", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "taxation";
