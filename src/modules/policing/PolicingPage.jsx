...
import data from "././policing.data";
...
export default function PolicingPage() {
  const { liveIssues, loading } = useSupabaseModule("policing");
  const [tabId, setTabId] = useState(data.tabs?.[0]?.id || "overview");
  const activeTab = data.tabs?.find((t) => t.id === tabId) || data.tabs?.[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("policing", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "policing";
