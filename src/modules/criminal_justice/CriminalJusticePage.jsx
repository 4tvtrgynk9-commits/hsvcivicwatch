...
import data from "././criminal_justice.data";
...
export default function CriminalJusticePage() {
  const { liveIssues, loading } = useSupabaseModule("criminal_justice");
  const [tabId, setTabId] = useState(data.tabs?.[0]?.id || "overview");
  const activeTab = data.tabs?.find((t) => t.id === tabId) || data.tabs?.[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("criminal_justice", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "criminal_justice";
