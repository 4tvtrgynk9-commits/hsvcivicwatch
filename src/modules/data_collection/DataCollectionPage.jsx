...
import data from "././data_collection.data";
...
export default function DataCollectionPage() {
  const { liveIssues, loading } = useSupabaseModule("data_collection");
  const tabs = [
    ...(data.tabs || []).filter((tab) => tab.id === "surveillance"),
    ...(data.tabs || []).filter((tab) => tab.id !== "surveillance"),
  ];
  const [tabId, setTabId] = useState(tabs[0]?.id || "surveillance");
  const activeTab = tabs.find((t) => t.id === tabId) || tabs[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("data_collection", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "data_collection";
