...
export default function EnvironmentPage() {
  const { liveIssues, loading } = useSupabaseModule("environment");
  const [tabId, setTabId] = useState(data.tabs?.[0]?.id || "overview");
  const activeTab = data.tabs?.find((t) => t.id === tabId) || data.tabs?.[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("environment", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "environment";
