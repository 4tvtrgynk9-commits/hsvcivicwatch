...
export default function HousingCrisisPage() {
  const { liveIssues, loading } = useSupabaseModule("housing_crisis");
  const [tabId, setTabId] = useState(data.tabs?.[0]?.id || "overview");
  const activeTab = data.tabs?.find((t) => t.id === tabId) || data.tabs?.[0];
  const { statBlocks: liveStatBlocks, loading: statBlocksLoading, error: statBlocksError } = useModuleStatBlocks("housing_crisis", tabId);
  ...
  useEffect(() => {
    try {
      ...
      const moduleMatch = saved.module === "housing_crisis";
