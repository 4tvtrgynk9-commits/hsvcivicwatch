...
export default function WhatYouCanDo({ data }) {
  ...
  return (
    <section style={{ marginTop: 18 }}>
      <h3 style={{ marginTop: 0, marginBottom: 12, color: COLORS.navy, fontSize: 22 }}>
        What You Can Do
      </h3>

      {intro ? (
        <p style={{ color: COLORS.text, lineHeight: 1.75, fontSize: 16, marginTop: 0 }}>
          {intro}
        </p>
      ) : null}
      ...
      {contacts.length ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: COLORS.navy, marginBottom: 10 }}>Who to Contact</div>
      ...
      {meetings.length ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: COLORS.navy, marginBottom: 10 }}>Meetings, Elections, & Oversight</div>
          <div style={{ display: "grid", gap: 10 }}>
            {meetings.map((m, i) => (
              <div key={i} style={{ background: "white", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 14 }}>
      ...
      {paths.length ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: COLORS.navy, marginBottom: 10 }}>Complaint, Request, & Report Paths</div>
          <div style={{ display: "grid", gap: 10 }}>
            {paths.map((p, i) => (
              <div key={i} style={{ background: "white", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 14 }}>
      ...
      {actions.length ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: COLORS.navy, marginBottom: 10 }}>Take Action</div>
      ...
      {comparisons.length ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: COLORS.navy, marginBottom: 10 }}>What Other Places Have Done</div>
